...
// Streams
const httpCombinedStream = fs.createWriteStream(path.join(__dirname, 'logs', HTTP_LOG_FILE || 'http-combined.log'), {
  flags: 'a',
});
const httpErrorStream = fs.createWriteStream(path.join(__dirname, 'logs', HTTP_ERROR_LOG_FILE || 'http-error.log'), {
  flags: 'a',
});
